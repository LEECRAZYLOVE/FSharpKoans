...
module ``08: The Good Kind of Discrimination`` = 
    ...
    let ``02 Creating & pattern-matching a discriminated union`` () = 
        ...
        randomOpinion (BSc (Philosophy, Management)) |> should equal "!!SCIENCE!!"
    ...
    let ``04 A discriminated union can refer to itself (i.e., it can be recursive).`` () =
        ...
        let a = Node ("1", Node ("2", Node ("3", Node ("4", Empty, Empty), Empty), Empty), Empty)  // <-- you may want to spread this over multiple lines and/or let-bindings ...!
